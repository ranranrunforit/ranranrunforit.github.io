...
excerpt: "Prompt engineering Quick Study Notes. <br/><img src='/images/gaia.png'>"
collection: projects
date: 2025-05-12
category: pythons
tags:
...
A zero-shot prompt is the simplest type of prompt. It only provides a description of a task and some text for the LLM to get stafied with. This input could be anything: a question, a stafi of a story, or instructions. The name zero-shot stands for ’no examples’.

Let’s use Vefiex AI Studio (for Language) in Vefiex AI,6 which provides a playground to test prompts. In Table 1, you will see an example zero-shot prompt to classify movie reviews.

The table format as used below is a great way of documenting prompts. Your prompts will likely go through many iterations before they end up in a codebase, so it’s impofiant to keep track of your prompt engineering work in a disciplined, structured way. More on this table format, the impofiance of tracking prompt engineering work, and the prompt development process is in the Best Practices section later in this chapter (“Document the various prompt attempts”).
